...
// ==========================================
// CONFIGURAÇÃO SUPABASE (Mantendo suas chaves reais)
// ==========================================
...
export default function ReaderScreen() {
  ...
  useEffect(() => {
    const fetchBookData = async () => {
      setLoading(true);
      try {
        // Simulação de acesso - garantindo que entre no app
        setHasAccess(true); 
        ...
        if (fetchedChapters && fetchedChapters.length > 0) {
          ...
          // GARANTIA: Seleciona o primeiro capítulo da lista
    ...
    <div className={`min-h-screen pb-28 transition-colors duration-500 ${isDarkMode ? 'bg-gray-950 text-gray-300' : 'bg-[#F9F7F2] text-gray-800'}`}>
      ...
      <div className={`fixed inset-y-0 right-0 w-80 max-w-[85vw] bg-purple-950 shadow-2xl z-[70] transform transition-transform duration-300 ${isIndexOpen ? 'translate-x-0' : 'translate-x-full'} flex flex-col`}>
        ...
        <div className="flex-1 overflow-y-auto">
          {chaptersList.map((chapter) => (
            <button key={chapter.id} onClick={() => handleChapterSelect(chapter)} className={`w-full text-left px-6 py-4 border-b border-purple-900/30 ${currentChapter?.id === chapter.id ? 'bg-purple-900 text-yellow-500' : 'text-purple-100'}`}>
      ...
      {currentChapter ? (
        <main className="px-8 py-10 max-w-2xl mx-auto">
          ...
          {/* O SEGREDO DA DIAGRAMAÇÃO ESTÁ AQUI */}
          <article 
            className="text-justify leading-relaxed" 
            style={{ fontSize: `${fontSize}px`, hyphens: 'auto' }}
          >
            {/* Split por \n para garantir que os parágrafos se separem */}
        ...
        <div className="p-10 text-center">Nenhum conteúdo encontrado. Verifique seu banco de dados.</div>
      )}
      ...
      <div className={`fixed bottom-0 left-0 right-0 border-t flex items-center justify-center gap-10 py-4 px-6 backdrop-blur-lg z-40 ${isDarkMode ? 'bg-gray-950 border-gray-800' : 'bg-white border-gray-100'}`}>
        <button onClick={decreaseFont} className="flex items-center text-gray-500"><Type size={16} />-</button>
        ...
        <button onClick={increaseFont} className="flex items-center text-gray-500"><Type size={20} />+</button>
      </div>
    </div>
  );
}
