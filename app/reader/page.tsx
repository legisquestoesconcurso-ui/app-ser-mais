...
export default function ReaderScreen() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-purple-950 flex flex-col items-center justify-center text-white text-center p-4">
        <div className="w-12 h-12 border-4 border-purple-500 border-t-yellow-500 rounded-full animate-spin mb-4"></div>
        <p className="uppercase tracking-widest text-xs">Carregando conteúdo...</p>
      </div>
    );
  }
    ...
    <div className={`min-h-screen pb-28 transition-colors duration-500 ${isDarkMode ? 'bg-gray-950 text-gray-300' : 'bg-[#F9F7F2] text-gray-800'}`}>
      ...
      <div className={`fixed inset-y-0 right-0 w-80 max-w-[85vw] bg-purple-950 shadow-2xl z-[70] transform transition-transform duration-300 ${isIndexOpen ? 'translate-x-0' : 'translate-x-full'} flex flex-col`}>
        <div className="p-6 border-b border-purple-800/50 flex items-center justify-between">
          <span className="text-yellow-500 font-bold text-xs tracking-widest">ÍNDICE</span>
      ...
      <header className="sticky top-0 z-50 bg-purple-950 text-white px-4 py-4 flex items-center justify-between shadow-md">
        ...
        <h1 className="text-xs font-bold tracking-[0.2em] uppercase">Tempo a Dois</h1>
      ...
      {currentChapter ? (
        <main className="px-8 py-10 max-w-2xl mx-auto">
          {currentChapter.url_imagem && (
            <img src={currentChapter.url_imagem} alt="Capa" className="w-full h-64 object-cover rounded-3xl mb-10 shadow-xl" />
          )}
          ...
          >
            {/* CORREÇÃO CRÍTICA: Split por \n para garantir que o texto apareça sempre */}
        ...
        <div className="p-10 text-center">Nenhum capítulo encontrado no banco.</div>
      )}

      {/* CONTROLES INFERIORES */}
      <div className={`fixed bottom-0 left-0 right-0 border-t flex items-center justify-center gap-10 py-4 px-6 backdrop-blur-lg z-40 ${isDarkMode ? 'bg-gray-950/90 border-gray-800' : 'bg-white/90 border-gray-100'}`}>
